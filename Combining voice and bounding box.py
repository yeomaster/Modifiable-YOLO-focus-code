...
# key value == name of class that is mapped to the number (ie 0== person, 1 == bicycle)q
...
# this function restarts the listening part of the system whenever we want to change the focus
def voice_listener():                                 
    """Background thread: keep listening until a valid YOLO item is spoken."""
    ...
    while True:   # keep looping until we succeed
        ...
        try:
            ...
            if spoken in items_lower:
                ...
                print("That’s not a YOLO item. Try again.")
        except sr.UnknownValueError:
            print("Could not understand audio. Try again.")
        except sr.RequestError as e:
            print(f"Speech API error: {e}")
            break          # stop if Google service fails
